...
classdef HHO
    ...
    properties
        ...
        cell_bases_Pkm1    % GM Addition: Basis functions of polynomial degree k-1 
        edge_bases      % The basis functions of the edges
        edge_bases_Pkm1 % GM Addition: The basis functions of degree k-1 the edges
    ...
    methods
        function obj = HHO(mesh_filename, K)
            ...
            for cell_i = 1:obj.mesh.ncells
             ...
             % GM Addition: Construct cell basis functions of degree k-1
            obj.cell_bases_Pkm1 = cell(obj.mesh.ncells,1);
            for cell_i = 1:obj.mesh.ncells
                obj.cell_bases_Pkm1{cell_i} = obj.createCellBasis_Pkm1(cell_i);
            end
            ...
            % Construct edge basis functions
            obj.edge_bases_Pkm1= cell(obj.mesh.nedges,1);
            for edge_i = 1:obj.mesh.nedges
                obj.edge_bases_Pkm1{edge_i} = obj.createEdgeBasis_Pkm1(edge_i);
            end            
            ...
            for cell_i = 1:obj.mesh.ncells
                ...
                edge_bases_for_cell_Pkm1 = obj.edge_bases_Pkm1(obj.mesh.cell_edges{cell_i});
                obj.elements{cell_i} = QuadratureElement(cell_i,vertices,center,K,cell_doe,edge_doe,obj.cell_bases{cell_i},obj.cell_bases_Pkm1{cell_i},edge_bases_for_cell,edge_bases_for_cell_Pkm1);
            end
        end
        ...
        %% GM Addition 
        function basis = createCellBasis_Pkm1(obj,cell_i)
            %createCellBasis Create a basis for polynomial functions on the
            % cell with the given index. Degree k-1 polynomial.           
            basis = cell((obj.K)*(obj.K+1)/2, 1);
            n = 1;
            for i = 0:(obj.K-1)
                for j = 0:i
                    xT = obj.mesh.cell_center(cell_i, :);
        ...
        function basis = createEdgeBasis_Pkm1(obj,edge_i)
            %createEdgeBasis Create a basis for polynomial functions on the
            % cell edge with the given index
            basis = cell(obj.K, 1);
            n = 1;
