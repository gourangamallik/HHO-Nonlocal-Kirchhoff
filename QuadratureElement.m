...
classdef QuadratureElement
    ...
    properties
        ...
        cell_values_in_cell_Pkm1  % GM Addition: Values of the cell basis functions of degree k
        cell_values_on_edge     % Values of the edge basis functions
        cell_values_on_edge_Pkm1  % GM Addition: Values of the cell basis functions of degree k
        ...
        edge_values_Pkm1          % GM Values of the edge basis functions of degree k-1
        nhighorder_dofs         % Number of K+1 cell degrees of freedom
        ncell_dofs              % Number of cell degrees of freedom
        ncell_dofs_Pkm1           % GM  Number of cell degrees of freedom of degree k-1
        nedge_dofs              % Number of edge degrees of freedom
        nedge_dofs_Pkm1         % Number of edge degrees of freedom for (k-1)
        ngrad_dofs              % Dimension of the gradient space
        ntotal_edge_dofs        % Total number of edge degrees of freedom
        ntotal_edge_dofs_Pkm1        % Total number of edge degrees of freedom of deg k-1
        ntotal_dofs             % Total number of degrees of freedom
        ntotal_dofs_Pkm1             % Total number of degrees of freedom of deg k-1
    end
    
    methods
        function obj = QuadratureElement(cell_id,vertices,center,K,cell_doe,edge_doe,cell_basis,cell_basis_Pkm1,edge_bases,edge_bases_Pkm1)
            ...
            obj.ncell_dofs_Pkm1 = (K)*(K+1)/2;
            obj.ngrad_dofs = (K+2)*(K+3)/2 - 1;
            obj.nedge_dofs = length(edge_bases{1});
            obj.nedge_dofs_Pkm1 = length(edge_bases_Pkm1{1});
            obj.ntotal_edge_dofs = obj.nedges * obj.nedge_dofs;
            obj.ntotal_edge_dofs_Pkm1 = obj.nedges * obj.nedge_dofs_Pkm1;
            obj.ntotal_dofs = obj.ncell_dofs + obj.ntotal_edge_dofs;
            obj.ntotal_dofs_Pkm1 = obj.ncell_dofs_Pkm1 + obj.ntotal_edge_dofs_Pkm1;
            ...
            % Compute quadrature for edge basis functions of degree k-1
            obj.edge_values_Pkm1 = zeros(obj.nedges, length(edge_bases_Pkm1{1}), obj.nedge_qnodes);
            for edge_i = 1:obj.nedges
                for b = 1:length(edge_bases_Pkm1{edge_i})
                    for iqn = 1:obj.nedge_qnodes
                        pt = obj.edge_quad_rules{edge_i}(iqn, 1:2);
                        obj.edge_values_Pkm1(edge_i,b,iqn) = edge_bases_Pkm1{edge_i}{b}.eval(pt(1), pt(2));
                    end
                end
            end 
            %% GM addition: Compute quadrature for cell basis functions of degree k-1 on the cell, used for (k-1) projection
            obj.cell_values_in_cell_Pkm1 = zeros(obj.nedges, length(cell_basis_Pkm1), obj.ncell_qnodes);
            for edge_i = 1:obj.nedges
                for b = 1:length(cell_basis_Pkm1)
                    for iqn = 1:obj.ncell_qnodes
                        pt = obj.cell_quad_rules{edge_i}(iqn, 1:2);
                        obj.cell_values_in_cell_Pkm1(edge_i,b,iqn) = cell_basis_Pkm1{b}.eval(pt(1), pt(2));
                    end
                end
            end
            
           % GM addition: Compute quadrature for cell basis functions on
           % the edges of degree k-1
            obj.cell_values_on_edge_Pkm1 = zeros(obj.nedges, length(cell_basis_Pkm1), obj.nedge_qnodes);
            for edge_i = 1:obj.nedges
                for b = 1:length(cell_basis_Pkm1)
                    for iqn = 1:obj.nedge_qnodes
                        pt = obj.edge_quad_rules{edge_i}(iqn, 1:2);
                        obj.cell_values_on_edge_Pkm1(edge_i,b,iqn) = cell_basis_Pkm1{b}.eval(pt(1), pt(2));
                    end
                end
            end 
