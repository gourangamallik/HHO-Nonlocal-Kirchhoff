...
function [AS_TF,A_TF,G_KT] = DiffusionOperator( hho, cell_i)
...
% gradient reconstruction operator matrix G_KT
    ...
    grad_basis_offset = hho.getBasisDegreeOffset(1); % This is required as we need to exclude gradient of x^0y^0 (1)

    % Initialise matrices
    MG = zeros(element.ngrad_dofs); %(Stima for reconstruction operator)
    ...
    for edge_i = 1:element.nedges
        ...
        % hF = hho.mesh.edge_length(hho.mesh.cell_edges{cell_i}(edge_i)); kappa_F = 1;
        % STF = STF + (kappa_F / hF) * BRF' * edge_terms * BRF;
        hT = hho.mesh.h_size(cell_i); kappa_T = 1; 
        STF = STF + (kappa_T / hT) * BRF' * edge_terms * BRF; % hT is Better for thin meshes.
    end
