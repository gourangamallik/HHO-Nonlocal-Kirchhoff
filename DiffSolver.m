function xTF = DiffSolver( hho, AQ,bc)
...
for cell_i = 1:hho.mesh.ncells  
    ...
    a = AQ{cell_i};
    g_cell_index=(1+(cell_i-1)*element.ncell_dofs):cell_i*element.ncell_dofs;
    b = bc(g_cell_index);
    ...
    for i = 1:element.nedges
        ...
        for ik = 1:element.nedge_dofs
            ...
            g_edge_index=[g_edge_index;ipos];
            if all(dirichlet_edges~=i) % if non-dirichlet edge
                ...
                dirichlet_dof=[dirichlet_dof;ipos];
    ...
    A(g_edge_index,g_edge_index)=A(g_edge_index,g_edge_index)+AFF;
...
dofs=setdiff(1:nedge_dofs,unique(dirichlet_dof));
xF=zeros(nedge_dofs,1);
xF(dofs) = A(dofs,dofs) \ B(dofs);
